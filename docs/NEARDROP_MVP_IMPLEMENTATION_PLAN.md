...
| PATCH | `/api/v1/parcels/:id/status` | Carrier, Affiliate, Ops | Update parcel status |
...
12. Set up GitHub Actions CI pipeline (lint + typecheck + test)
...
- [ ] `npm run typecheck` — 0 TypeScript errors across all packages
- [ ] `npm run lint` — 0 ESLint errors across all packages
- [ ] `npm run test` — test infrastructure runs (even if 0 tests)
- [ ] `npm run migrate` — all migrations apply successfully
- [ ] `npm run migrate:rollback` — all migrations reverse successfully
- [ ] Health check endpoint returns 200 with DB connected
- [ ] `.env.example` documents ALL required variables
- [ ] README contains setup instructions that work from clean clone
...
**Next Step:** Approve all policy decisions (Section 14), then begin Phase 0
