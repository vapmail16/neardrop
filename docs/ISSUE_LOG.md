...
**2026-04-07 — Frontend DCDeploy parity with backend** — Needed the same style of Dockerfile, env checklist, deployment guide, and issue-log pattern. — Added `frontend/Dockerfile` (repo-root context), `output: 'standalone'` + `experimental.outputFileTracingRoot` in `next.config.mjs`, root `.dockerignore`, `frontend/DCDeploy_ENV_VARS.md`, `docs/FRONTEND_DEPLOYMENT_GUIDE.md`, checklist + deployment issue log updates, and a Vitest contract on `next.config`. — **Prevention:** Read `docs/FRONTEND_DEPLOYMENT_GUIDE.md` before changing `file:` shared path or Docker context.
