...
import { affiliateActionLabel, affiliateAllowedNextStatuses } from '@/lib/affiliate-transitions';
import { ApiRequestError } from '@/lib/api/client';

export function AffiliateParcelTable({
  parcels,
  onUpdated,
}: {
            ...
            {parcels.length === 0 ? (
              <tr>
                <td colSpan={5} className="px-4 py-8 text-center text-neutral-500">
                  No parcels assigned to your hub yet.
                </td>
              </tr>
            ) : (
              parcels.map((p) => {
                const nextStatuses = affiliateAllowedNextStatuses(p.status);
                return (
                  <tr key={p.id} data-testid={`affiliate-parcel-row-${p.id}`}>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex flex-wrap gap-2">
                        {nextStatuses.map((s) => (
                          ...
                          >
                            {busyId === p.id ? '…' : affiliateActionLabel(s)}
                          </button>
                        ))}
                      </div>
                    </td>
